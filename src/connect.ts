...
export const UNCLOCKED_DOCUMENTS: ReadonlySet<string> = new Set(["/openapi.json", "/apis.json", "/.well-known/api-catalog"]);
...
export function openApi(origin: string, now = Date.now()) {
  ...
  for (const r of SURFACE) {
    ...
    for (const v of verbs) {
      ...
      // Three JSON documents are served without the clock (UNCLOCKED_DOCUMENTS
      // above) and the description must not promise it there: the linkset
      // says so through its media type, the other two through this branch.
      const responseDesc =
        ...
        UNCLOCKED_DOCUMENTS.has(r.path) ? "JSON whose root belongs to another specification, served without now/now_utc: OpenAPI carries the instant as x-now/x-now_utc, APIs.json its own created/modified." :
