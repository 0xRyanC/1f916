...
const SEQUENCE_NOTE =
  "head is the checkpoints table's AUTOINCREMENT sequence. Every checkpointer pass consumes attempts_per_pass values (one INSERT OR IGNORE per log), written or ignored, so head advances on a quiet log where checkpoints[].id and created_at do not. Two reads of head that differ by attempts_per_pass prove the checkpoint step ran once between them; witness_dispatch.last_attempt_at proves only that the cron handler reached its later dispatch leg. attempted_pass_cron is the Worker schedule that runs the step, so Δhead / attempts_per_pass and Δt / the cron interval are two legs of one count and should agree: a shortfall in head is a missed pass, and a manual crank (POST /api/checkpoint) only adds, so it cannot hide one. Over one cron interval, a Δhead short of attempts_per_pass names the failing leg: attempts_per_pass − 1 means the second log's leg threw before its insert (there is no per-leg try, so the pass ends there); 0 means the first log's leg threw, or the step was never entered (REGISTRY_SEED unset).";
