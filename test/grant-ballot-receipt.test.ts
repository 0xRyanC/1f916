...
test("a voter still under the cap at the close is told the served figure is final, because the weight is pinned to voting_closes_at", async () => {
  // THE COHORT THE FIRST VERSION OF THIS RECEIPT WAS A FLOOR FOR, now fixed.
  // Before 91d446e8, tallyVotes weighed at the sponsor's actual close instant
  // (src/grants.ts:294 `now`), and grants.ts:289 refuses only an EARLY close,
  // so for a voter short of the cap at voting_closes_at a late close weighed
  // MORE tenure and calling the served number final was false. WQ-30 changed
  // tallyVotes to weigh at Math.min(now, voting_closes_at) (grants.ts:534), so
  // the weight is pinned at the declared close and a late close cannot raise
  // it. The receipt must now say so.
  //
  // KILLING MUTATION: in src/society.ts grantBallotFor, restore the old
  // "AT LEAST ... the tally weighs your tenure at the instant the sponsor
  // actually closes the vote ... can only be larger" clause. That reasserts a
  // floor the pinned weight no longer is, and the assertions below go red.
  const { env, db } = makeEnv();
  // Two days out: a one-hour-old citizen is still far short of the seven days
  // voteWeight needs to reach 1, so weight_at_close is under the cap.
  ...
  assert.doesNotMatch(receipt.ballot!.weight_note!, /AT LEAST/, "the weight is pinned at the declared close, not a floor");
  assert.match(receipt.ballot!.weight_note!, /final/, "an under-cap voter is now told the served figure is final");
  assert.match(
    receipt.ballot!.weight_note!,
    /declared close/,
    "and told why: the tally weighs tenure as of the declared close",
  );
});
